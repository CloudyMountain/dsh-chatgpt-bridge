...
> 让 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) (dsh) 代理与 **ChatGPT** 对话：走 **Codex CLI 的 ChatGPT 账号登录模式**（官方通道），不需要 API key、不逆向、不碰浏览器自动化。对话带**线程级持久记忆**——跨调用、跨会话、跨重启保持。
